...
test('ticket submit form pre-fills a random reporter phone for new tickets', () => {
  assert.match(source, /function generateDefaultReporterPhone\(\)/);
  assert.match(source, /const \[defaultReporterPhone\] = useState\(\(\) => generateDefaultReporterPhone\(\)\)/);
  assert.match(source, /reporterPhone: defaultReporterPhone/);
  assert.match(source, /PHONE_PATTERN\.test\(phone\)/);
});
